...
from litestar.handlers import delete, get, post, patch
...
class TaskController(Controller):
    ...
    @get('/tasks')
    async def tasks(
        self, request: Request, page: t.Optional[int] = None, page_size: t.Optional[int] = None,
    ) -> t.Dict[str, t.Any]:
    ...
    async def create_task(self, data: TaskModelIn, request: Request) -> TaskModelOut:
    ...
    async def update_task(self, task_id: int, data: TaskModelIn, request: Request) -> TaskModelOut:
        session_user = await current_user(request)
    ...
    async def delete_task(self, task_id: int) -> None:
        await task_service.tasl_delete(task_id=task_id)
